...
export default function UpcomingEventForm() {
  ...
  const handleEdit = () => {
    ...
    formik.setFieldValue("fromDate", dayjs(edit.event.fromDate).format('DD MMM YYYY'));
    formik.setFieldValue("toDate", dayjs(edit.event.toDate).format('DD MMM YYYY'));
  ...
  const checkTime = () => {
    ...
    if (startDate.isSame(endDate) && startTime.isAfter(endTime)) {
      dispacth(
        setSnackBar(
          {
            message: 'Start time must be before end time when the dates are the same',
            variant: 'error'
          }
        )
      );
    ...
    if (checkTime()) {
      try {
        ...
        if (!formik.values.isUpcoming) {
          dispacth(
            setSnackBar({
              message: "Event updated successfully and Add other details in Completed Event page",
              variant: "success",
            })
          );
        }
  ...
  const removeCoordinator = (id) => {
    setCoordinators(prevCoordinators => {
      const updatedCoordinators = prevCoordinators.filter(coordinator => coordinator.id !== id);
  ...
  const addCoordinator = async () => {
    try {
      ...
      if ((res.data.role === 'Coordinator' || res.data.role === 'admin') && check) {
                    ...
                    <ListItem >
                      <div style={{ color: "black" }}>
                        <div style={{ display: 'flex', flexDirection: "column", gap: "1rem" }}>
                          ...
                          <div>
                            <span style={{ fontSize: "18px", fontWeight: "bold", margin: "0px 5px" }}> Location : </span>
                            <span style={{ textAlign: 'justify' }}><a href={ele.githubLink}>{ele.location}</a></span>
                          </div>
                          <div>
                            <span style={{ fontSize: "18px", fontWeight: "bold", margin: "0px 5px" }}>Date and Time : </span>
                            <span style={{ textAlign: 'justify' }}>{ele.fromDate.slice(0, 10) + " To " + ele.toDate.slice(0, 10)}{" -" + ele.startTime + " - " + ele.endTime}</span>
                          </div>
                          <div>
                            <span style={{ fontSize: "18px", fontWeight: "bold", margin: "0px 5px" }}>Registration Link: </span>
                            <span style={{ textAlign: 'justify' }}><a href={ele.registrationLink}>{ele.registrationLink}</a></span>
                          </div>
                          <div>
                            <span style={{ fontSize: "18px", fontWeight: "bold", margin: "0px 5px", display: "block" }}>Coordiantors : </span>
                            {ele.coordinators.map((i) => {
                              return (<span style={{ textAlign: 'justify', display: "block", margin: "0px 20px" }}>{i.email}</span>)
                            })}
                          </div>
                          <div>
                            <span style={{ fontSize: "18px", fontWeight: "bold", margin: "0px 5px", display: "block" }}>Prize Details: </span>
                            {ele.prizeDetails.map((i) => {
                              return (<span style={{ textAlign: 'justify', display: "block", margin: "0px 20px" }}>{i.position} : {i.description}</span>)
                            })}
                          </div>
                          <div>
                            <IconButton
      ...
      <div className="sub-contact-container" style={{ margin: ".9rem" }}>
        ...
        <div className="contact-fields  add-carousel-item ">
          <form onSubmit={visible && edit.check ? handleUpdateEvent : handleAddEvent}>
            ...
            <div id="coordianator-details" style={{ "display": "flex", "flexDirection": "column", gap: "1rem", "border": "1px solid rgb(222,222,222)", "padding": "1rem" }}>
              <div id="coordinator-list">
                {coordinators && coordinators.map((item) => {
                  return (<div style={{ display: "flex", flexDirection: "row", justifyContent: "space-evenly", gap: "1rem", margin: "10px 0px" }}  >
                    <span >{item.email}</span><span style={{ fontWeight: "bold", color: "grey", cursor: "pointer" }} onClick={() => { removeCoordinator(item.id) }}>X</span>
                  </div>)
                })}
              </div>
              <input
            ...
            <input
              ...
              onChange={formik.handleChange}
            />
            {visible && edit.check && <FormControlLabel name="isUpcoming" onChange={(val) => formik.setFieldValue("isUpcoming", !val)} onClick={() => console.log(formik.values.isUpcoming)} control={<Checkbox />} label="Mark Event as Completed" />}
            {visible && edit.check ? (
              <button
